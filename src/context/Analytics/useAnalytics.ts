...
export type AnalyticEventName =
  ...
  | 'Tokens swapped'
  | 'Tokens staked in STRATA vault'
  | 'Token withdrawal requested from STRATA vault'
  | 'Token withdrawals executed from STRATA vault'
  | 'Tokens staked in SEUSD vault'
  | 'Tokens withdrawn from SEUSD vault'
  | 'Pool reward claimed'
  | 'SEUSD vault reward claimed'
  | 'STRATA vesting vault reward claimed'
  | 'Vote casted';
...
export type AnalyticEventProps<TEventName extends AnalyticEventName> =
  TEventName extends 'Tokens supplied'
    ...
    : TEventName extends 'Tokens staked in STRATA vault'
    ? {
        poolIndex: number;
        rewardTokenSymbol: string;
        tokenAmountTokens: number;
      }
    : TEventName extends 'Token withdrawal requested from STRATA vault'
    ? {
        poolIndex: number;
        rewardTokenSymbol: string;
        tokenAmountTokens: number;
      }
    : TEventName extends 'Token withdrawals executed from STRATA vault'
    ? {
        poolIndex: number;
        rewardTokenSymbol: string;
      }
    : TEventName extends 'Tokens staked in SEUSD vault'
    ? {
        tokenAmountTokens: number;
      }
    : TEventName extends 'Tokens withdrawn from SEUSD vault'
    ? {
        tokenAmountTokens: number;
      }
    ...
    ? {
        ...
        luTokenAddressesWithPendingReward: string[];
      }
    : TEventName extends 'STRATA vesting vault reward claimed'
    ? {
        poolIndex: number;
        rewardTokenSymbol: string;
      }
