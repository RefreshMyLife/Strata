...
const initializeYup = () => {
  ...
  yup.addMethod(
    ...
    function (minValue: BigNumber | string | number | undefined, errorMessage: string) {
      return this.test('positive', errorMessage, function (value: string | number | undefined) {
        // Mark value as valid if no minValue has been defined
        ...
        const isGreaterThanOrEqualToMinValue =
          isValueValid && new BigNumber(value).isGreaterThanOrEqualTo(minValue);

        const isValid = isValueValid && isGreaterThanOrEqualToMinValue;
