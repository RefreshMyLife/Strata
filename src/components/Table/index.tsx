...
export function Table<R>({
  ...
  return (
    <Paper css={styles.getRoot({ breakpoint, noPadding: withPadding === true })} className={className}>
      ...
      <MuiTableContainer css={styles.getTableContainer({ breakpoint })} >
        <MuiTable css={styles.table({ minWidth: minWidth ?? '0', padding: withPadding })} aria-label={title}>
          <Head
            columns={columns}
            ...
            {sortedData.map((row, rowIndex) => {
              ...
              return (
                <MuiTableRow
                  hover={hover}
                  key={rowKey}
                  css={[styles.link, styles.getTableRow({ clickable: !!rowOnClick, padding: withPadding })]}
                  onClick={
      ...
      <TableCards
        ...
        rowWrapperMobile={rowWrapperMobile}
        rowTitleMobile={rowTitleMobile}
