      ...
      <div>
        {data.map((row, rowIndex) => {
          ...
          return (
            ...
            >
              <div css={[styles.rowTitleMobile, rowTitleMobile]}>{titleColumn.renderCell(row, rowIndex, params)}</div>

              <Delimiter css={styles.delimiterMobile} />

              <div className="table__table-cards__card-content" css={[styles.rowWrapperMobile, rowWrapperMobile]}>
                {otherColumns.map(column => (
                  <div key={`${rowKey}-${column.key}`} css={styles.cellMobile}>
                    ...
                    <Typography variant="small2" css={styles.cellValueMobile}>
                      {column.renderCell(row, rowIndex, params)}
                    </Typography>
                  </div>
                ))}
              </div>
