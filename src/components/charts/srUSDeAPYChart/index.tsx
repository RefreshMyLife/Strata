...
export interface srUSDeAPYChartItem {
  date: string;
  value: number;
}

export interface srUSDeAPYChartProps {
  data: srUSDeAPYChartItem[];
  color?: string;
  className?: string;
}

const CustomTooltip = ({ active, payload, chartColor }: any) => {
...
export const SrUSDeAPYChart: React.FC<srUSDeAPYChartProps> = ({
  className,
  data,
  color
}) => {
  ...
  // Blue color scheme for srUSDeAPY
  const chartColor = color || 'rgba(33, 154, 253, 1)';
  const baseId = useUID();
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <YAxis
            ...
            ticks={[domainMin, step3, step2, step1, domainMax]}
            width={40}
            tickCount={5}
            interval={0}
            tickMargin={localStyles.tickMargin}
          ...
          <Bar
            ...
            shape={(props: any) => {
              ...
              return (
                <g>
                  <rect x={x} y={y} width={width} height={height} fill="rgba(33, 154, 253, 0.1)" rx={2} />
                  <line x1={x} y1={y} x2={x + width} y2={y} stroke="rgba(33, 154, 253, 1)" strokeWidth={1} />
                  {isActive && (
                    <circle
                      cx={x + width / 2}
                      cy={y}
