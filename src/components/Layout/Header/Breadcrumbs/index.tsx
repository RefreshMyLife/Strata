...
import { getLuTokenByAddress } from 'utilities';
    ...
    return activeRoute.subdirectories.reduce<PathNode[]>((acc, subdirectory) => {
      ...
      switch (subdirectory) {
        ...
        case Subdirectory.MARKET: {
          hrefFragment = Subdirectory.MARKET.replace(':luTokenAddress', params.luTokenAddress);

          const luToken = getLuTokenByAddress(params.luTokenAddress);

          if (luToken) {
            dom = (
              <div css={styles.tokenSymbol}>
                <span>{luToken.underlyingToken.symbol}</span>

                {!!accountAddress && canRegisterToken() && (
                  <TertiaryButton
                    css={styles.addTokenButton}
                    onClick={() => addTokenToWallet(luToken.underlyingToken)}
                  >
        ...
        case Subdirectory.VAULTS:
          ...
          break;
        case Subdirectory.PRE_DEPOSIT:
          dom = t('breadcrumbs.preDeposit');
          break;
        case Subdirectory.HISTORY:
          dom = t('breadcrumbs.history');
          break;
        case Subdirectory.STRATA:
          dom = t('breadcrumbs.strata');
          break;
        case Subdirectory.CONVERT_SRT:
          dom = t('breadcrumbs.convertSrt');
          break;
        case Subdirectory.SWAP:
          dom = t('breadcrumbs.swap');
          break;
        case Subdirectory.SEUSD:
          dom = t('breadcrumbs.seusd');
          break;
        default:
          break;
      }
