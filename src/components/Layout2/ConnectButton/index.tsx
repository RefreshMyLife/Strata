...
export const ConnectButtonUi: React.FC<ConnectButtonProps> = ({
  ...
  const handleOpenEtherscan = () => {
    const url = generateBlockchainScanUrl({ hash: accountAddress, urlType: 'address', chainId: config.chainId });
  ...
  if (accountAddress) {
    return (
      <>
        ...
        >
          <SvgWallet2 />
          <span css={styles.walletAddressSpan}>{accountNs || truncateAddress(accountAddress)}</span>
          <CheveronDown css={[styles.chevronButton, { transform: isMenuOpen ? 'rotate(180deg)' : 'rotate(0)' }]} />
        </Button>
        ...
        >
          ...
          <MenuItem onClick={handleOpenEtherscan} >
            Open on Etherscan
          </MenuItem>
